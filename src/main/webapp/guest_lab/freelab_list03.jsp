	...
	<CENTER>
		<TABLE border=0 width=600 cellpadding=4 cellspacing=0>
			...
			<tr align="center" bgcolor="#87E8FF">
				<td width="50" bgcolor="#DFEDFF"><font size="2">번호</font></td>
				<td width="59" bgcolor="#DFEDFF"><font size="2">이름</font></td>
				<td width="84" bgcolor="#DFEDFF"><font size="2">폰</font></td>
				<td width="78" bgcolor="#DFEDFF"><font size="2">성별</font></td>
				<td width="340" bgcolor="#DFEDFF"><font size="2">주소</font></td>
			</tr>
			<tr align="center">
			...
			String en = null; //DB에서 mail 주소를 가져와서 처리하는 변수 
			...
			sql = "select * from guestlab order by idx";
			...
			if (!(rs.next())) { // rs의 값이 존재하지 않을때 
				...
				do {
					...
					idx.addElement(Integer.parseInt(rs.getString("idx")));
			...
			*/
			...
			<tr>
				...
				<td><a href="guestlab_read.jsp?id="> <%=ename.elementAt(j)%>
				</a></td>
		...
		if (wheregroup > 1) { //현재 나의 그룹이 1 이상일때 처음
			out.println("[<a href='freelab_list03.jsp?gogroup=1'>처음</a>]");
			out.println("[<a href='freelab_list03.jsp?gogroup=" + priorgroup + "'>이전</a>]");
		...
		if (ename.size() != 0) { //name.size() : 총 레코드의 갯수 가 0이 아니라면  
			for (int jj = startpage; jj <= endpage; jj++) {
				if (jj == where) { //jj 가 자신의 페이지 번호라면 링크 없이 출력
			out.println("[" + jj + "]");
				} else { //jj가 현재 자신의 페이지 번호가 아니라면 링크를 걸어서 출력
			out.println("[<a href=freelab_list03.jsp?go=" + jj + ">" + jj + "</a>]");
				}
			}
		}
		...
		if (wheregroup < totalgroup) { //링크를 처리
			out.println("[<A href=freelab_list03.jsp?gogroup=" + nextgroup + ">다음</A>]");
			out.println("[<A href=freelab_list03.jsp?gogroup=" + totalgroup + ">마지막]</A>");
			...
			<TABLE border=0 width=600 cellpadding=0 cellspacing=0>
				<TR>
					...
					<TD align=right valign=bottom width="117"><A
						href="guestlab_write.jsp"><img src="image/write.gif"
							border="0"></TD>
				</TR>
			</TABLE>
		</FORM>
